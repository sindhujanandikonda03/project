...
public class ChatClient extends JFrame implements ActionListener{
	...
	String userName = "yyang";//user name
	String topic = "test";
	...
	public void init(){
		...
		nickChange  = new JButton("Nickname" );
		...
		nickChange.setToolTipText("Change Nickname");
		fileTransfer.setToolTipText("Transfer a file");
		//add buttons to tool bar
		toolBar.add(userButton);
		toolBar.add(nickChange);
		...
		gridBagCon.gridy = 2;
		gridBagCon.anchor = GridBagConstraints.LINE_START;
		gridBag.setConstraints(channel_box,gridBagCon);
		downPanel.add(channel_box);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == userItem || obj == userButton) { //user config
			...
			UserConf userConf = new UserConf(this,userName, topic);
			userConf.setVisible(true);
			userName = userConf.userInputName;
			topic = userConf.topic;
		}
		else if(obj == nickChange){
			Nickname nickname = new Nickname(this, userName);
			nickname.setVisible(true);
			String old_nick = userName;
			userName = nickname.nick;
			SendCommand("Nick " + old_nick + " " + userName);
			//messageShow.append(old_nick + "changes nickname to  " + userName);
		}
		else if(obj == fileTransfer){
			System.out.println("fileTransfer");
			File file = new File("E:/IRC-master/IRC-master/1.txt");
			String sendTo = combobox.getSelectedItem().toString();
			TransferFile(file, sendTo);
			messageShow.append("Sending file to " + sendTo);
		}
		else if (obj == connectItem) { //connection config
			//pop up the window
	...
	public void Connect(){
		...
		try{
			...
			recvThread = new ClientReceive(socket,output,input,combobox,channel_box,messageShow,showStatus);
			recvThread.start();
	...
	public void TransferFile(File file, String user){
		...
		try{
		   ...
		   output.writeObject("SEND_FILE" + " " + user + " " + file.getName() + " " + fileSize);
	       ...
	       output.writeObject("SEND_COMPLETE");
	       output.flush();
	       fileStream.close();
